...
/** Event lists are long on a phone; three cards say enough, "Show all N" opens the rest. */
const EVENT_PHONE_ROWS = 3;
  ...
  return (
    <section aria-labelledby="fallback">
      ...
      <TableCard title={t('models.fallbackEvents.title')} icon={LifeBuoy} hintKey="models.fallbackEvents">
        ...
        <DataTable columns={eventColumns} rows={events} caption={t('models.fallbackEvents.title')} emptyText={t(fallbackEmptyKey(metric.period))} phoneRows={EVENT_PHONE_ROWS} />
      </TableCard>
      <Disclosure id="models.eras" label={t('models.eras.title')} defaultOpen={!isPhone}>
        <TableCard title={t('models.eras.title')} icon={History} hintKey="models.eras">
          <DataTable columns={eraColumns()} rows={data?.tables.eras ?? []} caption={t('models.eras.title')} limit={0} />
        </TableCard>
      </Disclosure>
      {models.length > 0 && (
        <Disclosure id="models.table" label={t('models.table.show', { n: fmt.int(models.length) })}>
          <TableCard title={t('models.table.title')} hintKey="models.table">
            <DataTable columns={modelColumns()} rows={models} caption={t('models.table.title')} limit={0} />
          </TableCard>
        </Disclosure>
      )}
    </section>
